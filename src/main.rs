...
async fn main() {
    ...
    let config = Config::init(); // Config::init reads the content of .env and creates the Config object

    let pool = match PgPoolOptions::new() //creation of the connection pool
        .max_connections(10)//pool opens 10 connections towards postgres
        .connect(&config.database_url) //it connects using the url in .env
        .await {
            Ok(pool) => {
